...
"All models are wrong, but some are useful" — a line usually attributed to the statistician George Box [@box1976]. The point is not that models are bad. The point is that the value of a model lies not in whether it is true but in whether it helps you make better decisions. The question for this section is not "what is the LLM really doing inside?" — that question is open and may remain open. The question is: which mental model of LLM behavior helps a programmer get better results from it?
