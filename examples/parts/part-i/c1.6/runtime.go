...
// Package agent is the runtime as section 6.6 leaves it, which is where Part
// I ends: a tool boundary, a policy check on writes, a gate feeding
// consequences back, and a memory that survives the process -- the states and
// wiring declared in one profile, the behavior supplied by one runtime.
//
// Listing 6.1 extracts from this package.
package agent
...
// every tool built from the profile's declarations, and Start and End resolve
// the notes file against it too.
